# Homework 2: Histograms and Distributions
## Due: Friday, 09.11.2026 at 11:59 PM ET
...
`python3 -mpip install --user scipy==1.13.1 # install the scipy module with version number 1.13.1 `
...
`conda install scipy=1.13.1   # Install the scipy version 1.13.1`

`conda update scipy          # Update the module scipy`

> Note: In order to specify a specific version number `pip` uses `==` while `conda` uses `=`.

To complete this homework, you will need to install the following modules with the specified versions:
1. `numpy==1.26.4`: this is a module that provides array and matrix classes, and many mathematical operations on those classes. It is the foundation of many of the modules that are used in data science.
2. `scipy==1.13.1`: this module provides many other useful functions for data analysis, including functions for dealing with probability distributions.
3. `matplotlib==3.9.2`: a basic plotting/visualization library. We will use this library to create a histogram in problem 1.

> Note 1: The autograder runs with exactly these versions. If you already have newer versions installed they will very likely work too (the functions this homework uses have stable behavior), but if you run into a mismatch, create a fresh environment with the exact versions listed above.
...
> Note: The `plot=plt` argument above is only so you can visualize the plot while experimenting. In your `get_coordinates` implementation, call `stats.probplot(data, dist=each_dist)` **without** the `plot` argument -- you only need the returned coordinates.
...
Download the files for HW 2 from Brightspace.
...
3. `hw2.py`, a skeleton script for Problem 1 and 2
4. `testbin.py`, to help individually test the functions you will write for Problem1

## Problem 1: Histogram Bin Width Optimization
...
Histogram: [2   4   5   2  7] (For this example we used 5 bins) 
    (This is what is the input to the norm_histogram function)
Number of samples: 20   (added up the samples: 2 + 4 + 5 + 2 +7 = 20)
Normalized list of probabilities: [0.1   0.2   0.25  0.1  0.35]    
(the first bin of the original histogram which has 2 is 10% of the 
    total number of samples(20).
The second bin of the original histogram has 4 samples which is 20% 
    of the total number of samples (20), etc.)
```
...
4. `find_min` is a generic function that takes a list of numbers and returns the three smallest numbers in that list along with their index. More specifically, the function returns a dictionary, i.e. {index_of_the_smallest_value: the_smallest_value, index_of_the_second_smallest_value: the_second_smallest_value, index_of_the_third_smallest_value: the_third_smallest_value}. For example, if the input is [14, 27, 15, 49, 23, 41, 147], then you should return {0: 14, 2: 15, 4: 23}.
...
normalized histogram:  [0.005, 0.02, 0.049, 0.145, 0.201, 0.234, 
    0.21, 0.087, 0.037, 0.012]
```
...
If your functions all work, and you run the test code that is included in `hw2.py` by uncommenting the problem 1 test part, you should produce the following output: `{14: -0.01868362283870376, 17: -0.01863540572320631, 20: -0.018591577843945776}`. 

> The `if __name__ == '__main__'` line in `hw2.py` is a useful way to write tests for your code: this is code that will *only* run if you run this file as the main script; if this file is included from another script, this test code will not run.
...
> Note: for this problem we will assume that the best fit distribution is that in which the sum of distances from the coordinates of the probplot to the identity line (X=Y) is minimized.
...
3. Complete the `find_dist` function. This function takes in a dictionary of distribution names and their respective errors (sum of distances). Your code must find the minimum value in the values of the dict and the corresponding key. Returns a tuple that contains the distribution selected and the error calculated. For example `('norm', 9.87546)`.


If your functions all work, and you run the test code that is included in `hw2.py` by uncommenting the problem 2 test part and commenting the problem 1 part, then if your code is correct you should get the following results for the files  `sample_norm.csv`, `sample_expon.csv`,`sample_uniform.csv`, `sample_wald.csv`, `distA.csv`, `distB.csv`, `distC.csv` respectively:
```
...
```
> **Important Note**: We will only check your values upto 4 decimal places. Values of data type np.float64 instead of float will also be accepted; for example, both `-0.01868362283870376` and `np.float64(-0.01868362283870376)` will be accepted.

# What to Submit
Please submit ONLY `hw2.py` with all the appropriate functions filled in for Problem1 and Problem2. If you are building your own test cases, make sure you place all code under `if __name__ == "__main__"`. Also, do not modify the imports (do not put additional imports except numpy, scipy, and matplotlib). Failure to do so or having any additional print statements outside main will lead to autograder crash. 
